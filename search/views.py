...
def ajax_solr_search(request, kind, model, order):
	...
	results = SearchQuerySet().using("flyover").filter(text=query).models(Doctor, Specialty, Service).order_by('last_name', 'specialty', 'name')
	count = len(results)

	suggestion = results.spelling_suggestion()
	if count==0 and suggestion is not None:
		suggestion = str(suggestion).split('(')[-1].strip(')')
		results = SearchQuerySet().using("flyover").filter(text=suggestion).models(Doctor, Specialty, Service).order_by('last_name', 'specialty', 'name')
		count = len(results)
